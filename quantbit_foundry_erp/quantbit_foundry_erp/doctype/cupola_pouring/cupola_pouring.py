...
class CupolaPouring(Document):
	...
	def validate_poring_weight(self):
		if self.total_consumed_weight < self.total_pouring_weight:
			frappe.throw(f'"Total Cupola Pouring Weight" must be less than "Total Consumed Weight"')
	...
	def validate_stock(self):
		for vsk in self.get("change_mix_details",filters = {"quantity" :['!=',0]}):
			if vsk.quantity > vsk.stock :
				frappe.throw(f'There is not enough stock present in warehouse "{vsk.warehouse}" of item "{vsk.item_name}" to proceed with cupola pouring entry')

		for stk in self.get("core_details"):
			if stk.qty > stk.stock :
				frappe.throw(f'There is not enough stock present in warehouse "{stk.warehouse}" of item "{stk.item_name}" to proceed with cupola pouring entry')
	...
	def manifacturing_stock_entry(self):
		for cd in self.get("casting_details"):      
			...
			se.custom_cupola_pouring = self.name	
			se.insert()
			se.save()
			se.submit()
	...
	def set_last_power_consumption(self):
		power_consumption = frappe.get_all('Cupola Pouring',
                                        filters ={'docstatus':1 ,},
	...
	def validate_last_power_consumption(self):
		...
		if self.power_reading_initial and last_doc:
			if self.power_reading_initial < last_doc :
				frappe.msgprint("'Power Reading Initial' must be greater than last sumitted Cupola Pouring`s 'Power Reading Final'")
	...
	# FoR Fetch Child entries from Heat Process Template
	@frappe.whitelist()
	def get_template_entries(self):
		if self.select_cupola_template:
			doc = frappe.get_all("Cupola Heat Process Template", 
							filters={"parent": self.select_cupola_template},
							fields=["process_id","process_name"],)
			if(doc):
				for d in doc:
					self.append('cupola_heat_details', {
